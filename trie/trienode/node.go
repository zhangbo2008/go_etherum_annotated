...
type Node struct { // 存哈希值和数据, 数据是一个字符串.
...
func (n *Node) IsDeleted() bool {
	return n.Hash == (common.Hash{}) // 删除的节点的哈希值是空hash对象,也就是一个空字符串.
}
...
type leaf struct { // 叶子节点是一个特殊的节点. 他保存他parent节点的哈希值.
...
type NodeSet struct { // 提交操作时候, 这个set里面放入了一系列的nodes.
	Owner   common.Hash
	Leaves  []*leaf
...
// 便利set里面的所有节点. 所有的路径里面按照长的先遍历. 路径越长也就是越bottom,
...
// AddNode adds the provided node into set.   //设置path的节点是n
...
// Merge adds a set of nodes into the set.  //把nodes里面节点放到set里面.
...
// we get rid of it?   //blob 信息构建为一个leaf, 然后放到set里面.
...
// we get rid of it?  //nodes里面所有哈希值组成的数组返回.
