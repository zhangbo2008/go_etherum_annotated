...
// act as a cache for another trie.Database. // 用来存节点的数据库.
...
func (db *ProofSet) Put(key []byte, value []byte) error {
	...
	db.nodes[keystr] = common.CopyBytes(value) // 作为value, 要拷贝一份再给nodes赋值, 不然的话是拷贝的指针. 就改原始数据了, 不是我们要的效果.
