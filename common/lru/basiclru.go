...
// The zero value is not valid, instances must be created using NewCache.   // comparable是可比较的类型.// (booleans, numbers, strings, pointers, channels, arrays of comparable types,
// structs whose fields are all comparable types).
type BasicLRU[K comparable, V any] struct { // lru就是最刚才用的放在链表最开始即可.
	list  *list[K]
	items map[K]cacheItem[K, V] // items 是从k 到cacheitem的映射.
	cap   int
}
...
type list[T any] struct { // list表示listElem链表的表头.结构体里面的泛型,用[]表示. 表示里面可以添加任意类型.T可以表示任意类型.
	root listElem[T]
}
...
func newList[T any]() *list[T] {
	l := new(list[T]) //new生成出来的东西是指向这个东西的指针.
	l.init()
	return l
}
...
func (l *list[T]) appendTo(slice []T) []T { //便利l里面所有元素,然后把他们都append到一个slice里面,返回这个slice
