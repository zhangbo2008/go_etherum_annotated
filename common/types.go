...
// Big converts a hash to a big integer. 把h转化为一个大整数.
...
func (h Hash) Generate(rand *rand.Rand, size int) reflect.Value {
	m := rand.Intn(len(h))            // 随意个0到n 的整数m
	for i := len(h) - 1; i > m; i-- { // 从 len(h) 到m 的长度生成一个
		h[i] = byte(rand.Uint32()) //rand函数最小就是uint32.没有更小的了.所以只能凑活用.
	}
	return reflect.ValueOf(h)
}

// Scan implements Scanner for database/sql.
func (h *Hash) Scan(src interface{}) error { //src里面数据拷贝到h里面.
	srcB, ok := src.([]byte)
	if !ok {
...
func (h *Hash) UnmarshalGraphQL(input interface{}) error {
	var err error
	switch input := input.(type) { //获取输入变量的类型.
	case string:
...
// MarshalText encodes the hash as hex. //marshal元帅, 所以是序列化的意思.
...
type Address [AddressLength]byte //代表20byte来形容的账户.
