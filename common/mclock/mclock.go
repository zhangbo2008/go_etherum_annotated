...
package mclock //单调时间的实现. 单调时间指的是.不允许回调的时间.
...
type AbsTime int64 //我们定义这个类型AbsTime
...
func (t AbsTime) Add(d time.Duration) AbsTime {
	return t + AbsTime(d) //对d进行类型转化后加t
}
...
type Clock interface {
	Now() AbsTime
	Sleep(time.Duration)
	NewTimer(time.Duration) ChanTimer //新建一个计时器.
...
type Timer interface {
	...
	Stop() bool //倒计时结束时候自动触发这个stop函数.
}

// ChanTimer is a cancellable event created by NewTimer.
type ChanTimer interface { // newtimer新建一个计时器.
	Timer //Timer是他的父类.
	...
	Reset(time.Duration) //timer停止了那么我们就reset.
}
