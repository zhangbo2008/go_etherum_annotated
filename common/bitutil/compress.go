...
// bitset representation algorithm.  //稀疏bitset表示算法. BitSet即位图
func bitsetEncodeBytes(data []byte) []byte {
	...
	nonZeroBitset := make([]byte, (len(data)+7)/8) // 8位存一个值. BitSet即位图.
	nonZeroBytes := make([]byte, 0, len(data))

	for i, b := range data {
		if b != 0 {
			nonZeroBytes = append(nonZeroBytes, b)
			nonZeroBitset[i/8] |= 1 << byte(7-i%8) // 把数据压缩到一个位置中.
		}
	}
...
// to the output as the input slice may be longer.   // 反编码.
