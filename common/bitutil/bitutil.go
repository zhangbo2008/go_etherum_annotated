...
const wordSize = int(unsafe.Sizeof(uintptr(0))) //指针的长度.
...
func fastXORBytes(dst, a, b []byte) int {
	...
	if w > 0 {
		//a=*p //将指针*p指向的变量值赋给a
		// a=p //指针p指向某变量的地址赋给a
		dw := *(*[]uintptr)(unsafe.Pointer(&dst)) // 把指针unsafe.Pointer(&dst) 转化为*[]uintptr指针. 然后*运算是拿到这个指针指向的内容, 把内容给dw.
		aw := *(*[]uintptr)(unsafe.Pointer(&a))
		bw := *(*[]uintptr)(unsafe.Pointer(&b))
		for i := 0; i < w; i++ { //对每一个word进行xor
			dw[i] = aw[i] ^ bw[i]
		}
	}
...
func fastTestBytes(p []byte) bool { // 测试p二进制是否是0.f
	n := len(p)
	w := n / wordSize
	if w > 0 {
